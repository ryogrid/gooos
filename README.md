...
| SMP | Done (v2) | **Multi-processor goroutine scheduling.** Per-CPU storage via `IA32_GS_BASE`; per-CPU GDT/TSS; per-CPU runqueues with work stealing (TinyGo runtime patch extended); LAPIC timer at 100 Hz on all CPUs (calibrated against PIT); IOAPIC discovery + redirection table (IRQ0/IRQ1 routed to BSP); IPI wakeup for cross-CPU scheduling; spinlocks on page allocator + process maps. APs enter the TinyGo scheduler loop and run goroutines (kernel + Ring-3). See `impldoc/smp_*.md` for the design set |
