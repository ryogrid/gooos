...
| SMP | Done (v2 on TinyGo 0.40.1, BSP-only scheduling) | **Multi-processor infrastructure.** Per-CPU storage (`IA32_GS_BASE`), per-CPU GDT/TSS, per-CPU runqueues, LAPIC timer (BSP), IPI wakeup, spinlocks (page allocator, process maps, Queue, sleep/timer queues), per-CPU `currentTask`. APs boot via INIT-SIPI-SIPI, enter the TinyGo scheduler after boot-phase gating (`bspBootDone`), and idle via `waitForEvents` (`sti; hlt; cli`). `stealWork` helper is shipped in the patched runtime but **intentionally not called** from the scheduler's pop site; enabling it triggers the AP Ring-3 triple-fault below. **Remaining issue:** Ring-3 user code triple-faults when stolen by an AP (`iretq` on AP per-CPU TSS — needs QEMU+GDB hardware-level debugging). All goroutines currently run on BSP (CPU 0). The migration to TinyGo 0.40.1 + `scheduler_cooperative.go` preserves this contract; promotion to `scheduler=cores` (with live stealWork) is deferred to `TODO_SMP3.md` M3 after the Ring-3 AP fault is resolved. See `impldoc/smp_deferred_and_known_issues.md` and `impldoc/smp_migration_overview.md` for details |
